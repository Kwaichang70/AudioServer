              ...
              return (
                ...
                >
                  ...
                  {/* R01.2: the row still plays from here on click; the menu adds
                      play next and add to queue without replacing the queue. It
                      works on the same list the row does — only what can play
                      now (V12.1) — so an unavailable item never enters the queue. */}
                  {playableHere && (
                    <PlayActions
                      target={{
                        kind: 'track',
                        track: toTrackInfo(track),
                        list: playable.map((t) => toTrackInfo(t)),
                        index: playable.indexOf(track),
                      }}
                    />
                  )}
                  <button
