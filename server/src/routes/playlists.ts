...
/**
 * A whole list in order (R01.3, "save the queue as a playlist"). Each entry is
 * the same shape as a single add, so local tracks are snapshotted from the
 * library and external ones from what the client sends (V12.1).
 */
const addItemsSchema = z.object({ items: z.array(addTrackSchema).min(1).max(5000) });
...
// Add a track to a playlist, or a whole list at once
playlistsRouter.post('/:id/tracks', (req, res) => {
  if (req.body && Array.isArray(req.body.items)) {
    const list = addItemsSchema.safeParse(req.body);
    if (!list.success)
      return res.status(400).json({ error: 'ValidationError', issues: list.error.issues });
    const found = loadPlaylist(req, res, 'write');
    if (!found) return;
    // One transaction, in the order given. An item addItem refuses — a local
    // id that is not in the library, an external one without a title — is
    // left out and named, instead of failing the whole list or vanishing.
    const skipped: Array<{ trackId: string; reason: string }> = [];
    let added = 0;
    getRawDb().transaction(() => {
      for (const item of list.data.items) {
        try {
          addItem(found.id, item);
          added += 1;
        } catch (err) {
          if (err instanceof UnknownTrackError || err instanceof MissingSnapshotError) {
            skipped.push({ trackId: item.trackId, reason: err.message });
          } else {
            throw err;
          }
        }
    ...
    return res.json({
      data: { ok: true, trackCount: count, added, skipped: skipped.length, skippedItems: skipped },
    });
  }
